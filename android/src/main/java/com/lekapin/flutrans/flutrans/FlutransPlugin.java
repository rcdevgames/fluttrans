...
public class FlutransPlugin implements MethodCallHandler, TransactionFinishedCallback {
  ...
  void payment(String str) {
    try {
      ...
      // TransactionRequest transactionRequest = new TransactionRequest(System.currentTimeMillis() + "", json.getInt("total"));
      TransactionRequest transactionRequest = new TransactionRequest(json.getString("orderId"), json.getInt("total"));
      ...
      Log.d(TAG, "First Name" + cus.getFirstName());
      Log.d(TAG, "Last Name" + cus.getLastName());
      Log.d(TAG, "Email" + cus.getEmail());
      Log.d(TAG, "Phone" + cus.getPhone());
